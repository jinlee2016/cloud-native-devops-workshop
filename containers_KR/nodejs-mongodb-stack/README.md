...
## Wercker를 사용하여 Node.js-MongoDB 컨테이너 패키지 애플리케이션을 빌드하고 Oracle Container Cloud Service에 배포합니다. ##
...
먼저 계정을 사용하여 [https://github.com] (https://github.com)에 로그인하고 [https://github.com/nagypeter/nodejs-mongodb &#39; -crud] (https://github.com/nagypeter/nodejs-mongodb-crud). Github 계정이없는 경우에는 [가입]하십시오 (https://github.com/join?source=header-home). 
...
이제 기본*build*파이프 라인이 시작되어*wercker.yml*에 정의 된 간단한*npm install*이 실행되어 Node.js 샘플 응용 프로그램에 필요한 노드 모듈을 얻습니다. 그 결과는 성공적 일 것입니다. 각 단계 (오른쪽)를 열면 세부 정보를 얻을 수 있습니다. 
...
**Action**버튼을 클릭하고**이 파이프 라인을 다시 실행**메뉴 항목을 선택하십시오. 
...
*포트***+ 추가**버튼이 나타나면 포트 매핑을 정의하려면 클릭하십시오. 이 포트 매핑을 사용하면 내부 도커 컨테이너의 포트 리디렉션을 호스트의 다른 포트로 사용할 수 있습니다. 호스트의 8091 포트에 매핑 할 내용을 3000에서 수신 대기하도록 구성된 Node.js 샘플 응용 프로그램. 
...
이제 Wercker는 서비스로 사용할 수있는 Node.js 샘플 애플리케이션 컨테이너를 만들었습니다. 스택을 생성 할 수 있도록 MongoDB가 기본적으로 생성됩니다. 왼쪽의**스택**메뉴를 클릭하고**새 스택**버튼을 클릭하십시오. 
...
여기서 컨테이너간에 (네트워크) 링크를 구성해야합니다.*Service*는 이전 단계에서 MongoDB 구성을 변경하지 않은 경우 MongoDB 인 정의 된 Container Cloud Service를 참조합니다. 마지막으로*Alias ​​*는 서비스의 호스트 이름입니다. 이것은 Node.js 샘플 응용 프로그램 서비스에서 MongoDB 인스턴스에 액세스하기 위해 MongoDB 드라이버를 구성하는 데 필요한 것입니다.**저장**을 클릭하십시오. 
...
응용 프로그램을 테스트하려면 호스트 환경의 공용 IP 주소를 얻어야합니다.*employee-nodejs*서비스가 배치 된 호스트 이름을 누르십시오. 

![alt text](images/45.stack.is.up.png)

public_ip 주소 속성을 찾아서 기록하십시오. 
...
빌드 및 푸시가 완료되면 Oracle Container Cloud Services 콘솔로 돌아가서 왼쪽 메뉴에서 컨테이너 항목을 선택하십시오.**name**에*employee-nodejs*문자열이 포함 된 컨테이너를 찾으십시오.**해당 컨테이너를 중지하십시오. 
...
*employee-nodejs-mongodb*스택 배치 세부 사항 페이지로 이동합니다. Oracle Container Cloud Service가 Docker 허브에서 이미지의 새 버전을 가져 오는 시점을 신속하게 확인할 수 있습니다. 서비스가 시작되어 실행될 때까지 기다린 후 세부 정보 페이지에 모든 것이 녹색으로 표시됩니다. 
